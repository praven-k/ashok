			...
			<div class="row">
				<div class="col-lg-3 wow slideInLeft">
					...
					<div class="service-tab-title">
						<ul class="clearfix">
						<li class="active" data-tab-name="staff">Staff Grievance Registration</li>

<li data-tab-name="student">Student&nbsp;Grievance&nbsp;Registration</li>
...
function generateid()
	{
		...
		$gid="GRRDS";
		$start=190001;
		$query = "select * from grievanceoregister where gid!=''";
		...
		$idgen= mysql_query("select max(SUBSTRING(gid,-6)) as idcount from grievanceoregister");
...
$cur=$ro444['dd'];
  ...
  $query1= "select max(Id) as cnt from grievanceoregister where mobile='".$mobile."'";
	//echo $query1;
...
$resul412 = mysql_query("update grievanceoregister set grievance='".$compliant."', rstatus=0 , gid='".$gid."', cur='".$cur."' where mobile='".$mobile."' and Id= '".$Id."'");


if($resul412)
		{

	$resul415 = mysql_query("select * from grievanceoregister where mobile='".$mobile."' and Id=(select max(Id) from grievanceoregister)");
...
$name = str_replace(' ', '%20', $name);
 $msg="Dear%20".$name."(".$regno."),%0AYour%20Grievance%20has%20been%20registered%20with%20ID%20".$gid."%0AAll%20future%20correspondence%20will%20be%20done%20through%20this%20id%20only.";
  ...
  $msg2="Dear%20Chairman-GRC,%20A%20Grievance%20has%20been%20registered%20with%20ID%20".$gid."%20from%20(".$regno.")";
...
$otp=$_POST['otp'];
$mobile=$_POST['mobile'];
$resul411 = mysql_query("select * from grievanceoregister where mobile='".$mobile."' and Id=(select max(Id) from grievanceoregister)");
// echo "select * from grievanceoregister where mobile='".$mobile."' and Id=(select max(Id) from grievanceoregister)";
...
if($ro411['otp']==$otp)
	...
	$result20=mysql_query("update grievanceoregister set status=1 where mobile='".$mobile."' and Id=(select max(Id) from grievanceoregister)");
...
<form action="" method="post">
					<div class="form-style-agile form-group" name="contact" onSubmit="return chklogfrm(contact.otp.value)" style="padding-top: -60px;">
						<label>
							Enter Correct OTP
...
<div id="committee">
		...
		include_once 'grievancecommittee.html';
		   ?>
							...
							<div id="Admin">
								<div class="col-lg-12 col-md-12 col-sm-12" style="margin-left: 60px;"><br><br><br>
									<?php
